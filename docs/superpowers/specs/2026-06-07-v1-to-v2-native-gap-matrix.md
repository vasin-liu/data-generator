...
| **AI P9 (shipped 2026-06-12)** | Per-provider/template scoped quotas (`quota.scope-overrides`), `ai_quota_scope_daily_usage`, `AiExecutionScope`, quota warn/exceed audit hooks (`AI_QUOTA_WARN` / `AI_QUOTA_EXCEEDED`), `verify-ai-p9.ps1` |
| **AI P10 (shipped 2026-06-17)** | Per-tenant scoped quotas (`metadata.tenantId` + `TENANT` overrides), quota notification webhooks (`quota.webhooks`), `verify-ai-p10.ps1` |
| **Next** | Quota webhook delivery retries, first-class tenant registry |

...
public class ClickHouseWriter<S extends WriteStageVO, T extends ClickHouseWriterVO> implements Writer<S, T> {
        ...
        try {
            ...
            if (stmt.isWrapperFor(ClickHouseStatement.class)) {
                ...
                chs.write()
                        ...
                        //.use("pd_dts")
                        .query(String.format(SQL_TEMPLATE, wvo.getTarget(), wvo.getTemplate()))
                        .set("format_csv_delimiter", Const.VERTICAL)
                        .data(DatasetKit.buildBulkData(wvo.getTemplate(), dataset, Const.VERTICAL, "\"\""))
                        //.table(wvo.getTarget())
                        .executeAndWait();
                return dataset.size();
            }

# Milestone v1.0: UDF, Transform & Test Harness
...
- [x] 02-01: Design and implement unified UDF registry model and persistence

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 02-02: Implement Java/PF4J and script UDF registration runtimes
- [x] 02-03: Implement SQL/Calcite UDF registration and governance integration
...
- v1 quality-first: harness and matrix before feature breadth
- Multi-form UDF (Java PF4J + script + SQL) with unified registry
- Transform v1 scoped to operators + SQL (not flow-control DAG)
- P0 regression gate = exactly 7 matrix rows; P1/P2 tracked but non-blocking
- Reader/Writer, datasource refactor, orchestration deferred to v2

**Issues Resolved:**

- Feature matrix and `verify-harness.ps1` CI entry established
- UDF registry, console upload/publish, template validation shipped
- json/mask/lookup operators + transform catalog API + run-report errors
- P0 matrix 7/7 green with merge gate in `harness-verify.yml`

**Issues Deferred:**

- No formal `/gsd-audit-milestone` run before close (accepted)
- Reader/Writer expansion, datasource refactor, template orchestration → v2
- Exhaustive matrix / UI coverage → future TEST-V2 requirements

**Technical Debt Incurred:**

- Milestone audit skipped at close
- Boot 4 / internal Kafka-ES starter compatibility (pre-existing)
- Console RBAC disabled by default for external deployments

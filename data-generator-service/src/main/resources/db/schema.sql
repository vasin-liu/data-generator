...
ALTER TABLE `task_execution` ADD COLUMN IF NOT EXISTS `connection_snapshot_json` CLOB;
...
-- Unified UDF artifact registry (Phase 3 D-01): one row per versioned UDF, global (no tenant column, D-03)
CREATE TABLE IF NOT EXISTS `udf_artifact`
(
    `id`            BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    `udf_id`        VARCHAR(256) NOT NULL,
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_udf_artifact_id_version` UNIQUE (`udf_id`, `version`)
);
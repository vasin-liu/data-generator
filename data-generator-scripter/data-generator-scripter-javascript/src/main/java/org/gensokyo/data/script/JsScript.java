...
public class JsScript<S extends ScriptStageVO, T extends JsScriptVO> implements Script<S, T> {
    ...
    public JsScript(ScriptFactory scriptFactory) {
    ...
    @Override
    public org.gensokyo.data.value.Value eval(final StageContext<S> ctx, final T spo,
        ...
        var dv = dataset.get();
        try (Context jsCtx = org.graalvm.polyglot.Context.newBuilder(LANGUAGE)
                .allowAllAccess(true)
            ...
            if (value.canExecute()) {
                var variables = scriptFactory.getVariables();
                for (Variable variable : variables) {
                    jsCtx.getBindings(LANGUAGE).putMember(variable.name(), variable.value());
                }
    ...
    private Source createScriptSource(String script) throws IOException {
        Source source = null;
        //远程文件链接
        if (StrKit.startWithIgnoreCase(script, "http")) {
            source = Source.newBuilder(LANGUAGE, new URL(script)).build();
        }
        //服务器本地文件
        if (Objects.isNull(source) && StrKit.endWithIgnoreCase(script, ".js")) {

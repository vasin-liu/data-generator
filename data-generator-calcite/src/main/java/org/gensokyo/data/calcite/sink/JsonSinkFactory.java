...
public class JsonSinkFactory implements V2SinkFactory {
    @Override
    public boolean supports(WriterVO writer) {
        return writer != null && Const.WriterType.JSON.equalsIgnoreCase(writer.getType());
    }

    @Override
    public RowSink create(WriterVO writer) {
        return new JsonRowSinkAdapter(writer);
    }
}

...
public class CalciteRowTransformer {
    ...
    public TransformResult transform(CalciteExecutionContext context) {
        ...
        List<Row> projected = filtered.stream()
                ...
                .toList();
        List<Row> distinct = applyDistinct(projected, select.isDistinct());
        List<Row> ordered = applyOrderByDistinct(distinct, plan.getOrderBy());
        List<Row> output = applyLimitAndOffset(ordered, plan.getOffset(), plan.getFetch());
    ...
    private List<Row> applyOrderByDistinct(List<Row> rows, SqlNodeList orderBy) {
        if (orderBy == null || orderBy.isEmpty()) {
            return rows;
        }
        List<Row> sorted = new ArrayList<>(rows);
        sorted.sort(orderDistinctComparator(orderBy));
        return sorted;
    }
    ...
    private Comparator<Row> orderDistinctComparator(SqlNodeList orderBy) {
        Comparator<Row> comparator = null;
        for (SqlNode node : orderBy) {
            Comparator<Row> next = distinctComparatorForOrderSpec(orderSpec(node));
            comparator = comparator == null ? next : comparator.thenComparing(next);
        }
        return comparator == null ? (left, right) -> 0 : comparator;
    }
    ...
    private Comparator<Row> distinctComparatorForOrderSpec(OrderSpec spec) {
        return comparingProjectedRows(spec.expression(), spec.ascending(), spec.nullsLast());
    }
    ...
    private Comparator<Row> comparingProjectedRows(SqlNode expression, boolean ascending, boolean nullsLast) {
        return (left, right) -> {
            Object leftValue = evaluateProjectedRow(left, expression);
            Object rightValue = evaluateProjectedRow(right, expression);
            int compared = compareValues(leftValue, rightValue, nullsLast);
            return ascending ? compared : -compared;
        };
    }
    ...
    private List<Row> applyDistinct(List<Row> rows, boolean distinct) {
        if (!distinct || rows.isEmpty()) {
            return rows;
        }
        Map<RowKey, Row> deduplicated = new LinkedHashMap<>();
        for (Row row : rows) {
            deduplicated.putIfAbsent(rowKey(row), row);
        }
    ...
    private Object evaluateProjectedRow(Row row, SqlNode node) {
        if (node instanceof SqlIdentifier identifier) {
            return resolveIdentifier(row, identifier);
        }
        return evaluate(row, node);
    }
    ...
    private long count(AggregateFrame frame, SqlBasicCall call, Map<String, SqlNode> aliasExpressions) {
        ...
        return aggregateValues(frame, call, aliasExpressions).size();
    }

    private BigDecimal sum(AggregateFrame frame, SqlBasicCall call, Map<String, SqlNode> aliasExpressions) {
        BigDecimal total = null;
        for (Object value : aggregateValues(frame, call, aliasExpressions)) {
    ...
    private BigDecimal avg(AggregateFrame frame, SqlBasicCall call, Map<String, SqlNode> aliasExpressions) {
        List<Object> values = aggregateValues(frame, call, aliasExpressions);
        BigDecimal total = null;
        ...
        return total.divide(BigDecimal.valueOf(values.size()), 8, RoundingMode.HALF_UP).stripTrailingZeros();
    }
    ...
    private Object extremum(AggregateFrame frame,
        ...
        Object best = null;
        for (Object candidate : aggregateValues(frame, call, aliasExpressions)) {
    ...
    private List<Object> aggregateValues(AggregateFrame frame,
                                         SqlBasicCall call,
                                         Map<String, SqlNode> aliasExpressions) {
        ...
        if (!aggregateDistinct(call) || values.isEmpty()) {
            return values;
        }
        Map<Object, Object> distinctValues = new LinkedHashMap<>();
        for (Object value : values) {
            distinctValues.putIfAbsent(normalizeComparable(value), value);
        }
        return List.copyOf(distinctValues.values());
    }

    private boolean aggregateDistinct(SqlBasicCall call) {
        SqlLiteral quantifier = call.getFunctionQuantifier();
        if (quantifier == null) {
            return false;
        }
        Object symbol = quantifier.getValue();
        return symbol == SqlSelectKeyword.DISTINCT;
    }
    ...
    private RowKey rowKey(Row row) {
        List<Object> values = new ArrayList<>(row.values().size());
        for (Object value : row.values().values()) {
            values.add(normalizeComparable(value));
        }
        return new RowKey(List.copyOf(values));
    }
}

# Phase 8 UAT — RW streaming CSV/JSON + JDBC upsert: Maven IT slice + optional Podman Playwright.
# CI merge-friendly gate: -SkipPlaywright runs embedded ITs only (D-28).
param(
    [switch]$SkipBuild,
    [switch]$SkipPlaywright,
    [switch]$KeepContainer,
    [string]$ImageTag = 'dg-phase8-rw-streaming-upsert-uat:local',
    [string]$ContainerName = 'dg-phase8-rw-streaming-upsert-uat',
    [int]$HostPort = 9876,
...
Write-Step "Phase 8 UAT — RW streaming CSV/JSON + JDBC upsert (Maven + Playwright)"

Write-Step "Maven slice — V2ScenarioTemplateIT, streaming/chunked pipelines, upsert SQL, validator (08-08..08-10)"
# OOM proof (CsvJsonStreamingOomIT) runs in a separate invocation with -Xmx256m — see step below (D-24).
$testList = @(
...
if ($code -ne 0) { throw "Phase 8 RW streaming Maven slice failed with exit code $code" }

Write-Step "Maven OOM proof — CsvJsonStreamingOomIT at -Xmx256m (D-06, D-24)"
# CsvJsonStreamingOomIT requires a reduced heap; do not apply -Xmx256m to the full slice above.
$ErrorActionPreference = 'Continue'
try {
    $oomCode = Invoke-RepoMaven -RepoRoot $RepoRoot `
...
if ($oomCode -ne 0) { throw "CsvJsonStreamingOomIT failed with exit code $oomCode (requires -Xmx256m)" }

if ($SkipPlaywright) {
    Write-Host "[SUCCESS] Phase 8 RW streaming & upsert Maven verification passed (-SkipPlaywright)." -ForegroundColor Green
    exit 0
}
...
try {
    ...
    Write-Step "Playwright E2E — rw-streaming-upsert.spec.ts (npm run e2e:phase8-rw-streaming-upsert)"
    ...
    try {
        ...
        npm run e2e:phase8-rw-streaming-upsert
    ...
    Write-Host ""
    Write-Host "[SUCCESS] Phase 8 RW streaming & upsert UAT automation passed." -ForegroundColor Green

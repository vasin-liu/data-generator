...
param(
    ...
    [string]$ApprovedBy = "",
    [ValidateSet("summary", "backlog", "signoff-status", "refresh", "batch-compare", "analyze", "draft", "compare", "promote", "signoff", "workflow", "workflow-promote")]
...
switch ($Action) {
    ...
    "workflow" {
        ...
        Invoke-MigrationApi -Method POST -Path "/migration/compare/$TemplateId" -Body '{"sampleSize":500}' | ConvertTo-Json -Depth 8
        Write-Host "Review report path and classification before signoff/promote." -ForegroundColor Yellow
    }
    "workflow-promote" {
        if ($TemplateId -le 0) { throw "TemplateId required for workflow-promote" }
        $inventoryId = "db-$TemplateId"
        Write-Host "=== analyze ===" -ForegroundColor Cyan
        Invoke-MigrationApi -Path "/migration/analyze/$TemplateId" | ConvertTo-Json -Depth 6
        Write-Host "=== draft ===" -ForegroundColor Cyan
        Invoke-MigrationApi -Method POST -Path "/migration/draft/$TemplateId" | ConvertTo-Json -Depth 4
        Write-Host "=== compare ===" -ForegroundColor Cyan
        $compare = Invoke-MigrationApi -Method POST -Path "/migration/compare/$TemplateId" -Body '{"sampleSize":500}'
        $compare | ConvertTo-Json -Depth 8
        $class = $compare.data.classification
        if ($class -eq "COMPATIBILITY_ONLY" -or $class -eq "BLOCKED") {
            throw "Classification $class — promote aborted"
        }
        Write-Host "=== signoff ===" -ForegroundColor Cyan
        $approver = if ([string]::IsNullOrWhiteSpace($ApprovedBy)) { "staging-operator" } else { $ApprovedBy }
        $signoffBody = @{ approved = $true; approvedBy = $approver } | ConvertTo-Json
        Invoke-MigrationApi -Method POST -Path "/migration/inventory/$inventoryId/signoff" -Body $signoffBody | ConvertTo-Json -Depth 6
        Write-Host "=== promote ===" -ForegroundColor Cyan
        Invoke-MigrationApi -Method POST -Path "/migration/promote/$TemplateId" | ConvertTo-Json -Depth 8
    }
}

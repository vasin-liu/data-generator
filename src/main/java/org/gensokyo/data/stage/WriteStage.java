    ...
    public Value internalExecute(Value input) {
        ...
        for (WriterPO wpo : stage.getWriters()) {
            try {
                ...
                throw new DataGeneratorException(String.format("模板 %s 执行数据写入阶段失败，写入类型为：%s ，目标数据源编号为：%s ，写入模板为：%s ，待写入值为：%s。",
                        ctx.template().getName(), wpo.getType(), wpo.getDataSourceId(), wpo.getTemplate(), JsonKit.write(data)), e);
            }
        }
